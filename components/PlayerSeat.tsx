...
export const PlayerSeat: React.FC<Props> = ({ player, isActive, isDealer, phase, positionStyle }) => {
  ...
  return (
    <div 
      className={`absolute flex flex-col items-center transition-all duration-300 ${player.status === PlayerStatus.FOLDED ? 'opacity-50 grayscale' : ''}`}
      style={positionStyle}
    >
      {/* Action Bubble */}
      {player.lastAction && (
      ...
      <div className="flex -space-x-4 mb-2 relative z-10">
        {player.hand.map((card, idx) => (
          <div key={idx} className={`transform transition-transform ${isActive ? '-translate-y-2' : ''}`}>
             <CardComponent card={card} hidden={hideCards} />
          </div>
        ))}
      </div>

      {/* Player Avatar Circle */}
      <div className={`relative w-16 h-16 md:w-20 md:h-20 rounded-full border-4 flex flex-col items-center justify-center shadow-lg bg-gray-800
        ${isActive ? 'border-yellow-400 ring-4 ring-yellow-400/30' : 'border-gray-600'}
        ${player.status === PlayerStatus.BUSTED ? 'bg-red-900' : ''}
      `}>
        <span className="text-xs text-gray-400 font-bold max-w-[90%] truncate text-center">
            {player.name}
        </span>
        <span className="text-xs md:text-sm text-white font-mono font-bold">
            ${player.chips}
        </span>
